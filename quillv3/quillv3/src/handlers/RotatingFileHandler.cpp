...
/***/
RotatingFileHandler::RotatingFileHandler(fs::path const& filename,
                                         RotatingFileHandlerConfig const& config,
...
/***/
void RotatingFileHandler::_clean_and_recover_files(fs::path const& filename, std::string const& open_mode, uint64_t today_timestamp_ns)
{
  if ((_config.rotation_naming_scheme() != RotatingFileHandlerConfig::RotationNamingScheme::Index) &&
