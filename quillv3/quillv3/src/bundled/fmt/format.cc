...
template FMTQUILL_API auto dragonbox::to_decimal(float x) noexcept -> dragonbox::decimal_fp<float>;
template FMTQUILL_API auto dragonbox::to_decimal(double x) noexcept -> dragonbox::decimal_fp<double>;
...
template FMTQUILL_API auto thousands_sep_impl(locale_ref) -> thousands_sep_result<char>;
...
template FMTQUILL_API void vformat_to(buffer<char>&, string_view, typename vformat_args<>::type, locale_ref);

// Explicit instantiations for wchar_t.

template FMTQUILL_API auto thousands_sep_impl(locale_ref) -> thousands_sep_result<wchar_t>;
