...
tm* gmtime_rs(time_t const* timer, tm* buf)
{
  ...
  {
    std::ostringstream error_msg;
    error_msg << "failed to call gmtime_rs, with error message " << "\"" << strerror(errno)
              << "\", errno \"" << errno << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
  return res;
...
tm* localtime_rs(time_t const* timer, tm* buf)
{
  ...
  {
    std::ostringstream error_msg;
    error_msg << "failed to call localtime_rs, with error message " << "\"" << strerror(errno)
              << "\", errno \"" << errno << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
  return res;
...
ReturnT callRunTimeDynamicLinkedFunction(const std::string& dll_name, const std::string& function_name, Args... args)
{
  ...
  {
    std::ostringstream error_msg;
    error_msg << "Failed to load library " << "\"" << dll_name << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
  ...
  if (QUILL_UNLIKELY(callable == nullptr))
  {
    ...
    std::ostringstream error_msg;
    error_msg << "Failed to call " << "\"" << function_name << "\" in " << "\"" << dll_name << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
  ...
  {
    std::ostringstream error_msg;
    error_msg << "Failed to free library " << "\"" << dll_name << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
...
void set_cpu_affinity(uint16_t cpu_id)
{
  ...
  {
    std::ostringstream error_msg;
    error_msg << "failed to call set_cpu_affinity, with error message " << "\"" << strerror(errno)
              << "\", errno \"" << errno << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
#endif
...
void set_thread_name(char const* name)
{
  ...
  {
    std::ostringstream error_msg;
    error_msg << "failed to call set_thread_name, with error message " << "\"" << strerror(errno)
              << "\", errno \"" << errno << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
#else
...
void* alloc_aligned(size_t size, size_t alignment, bool huge_pages /* = false */)
{
  ...
  {
    std::ostringstream error_msg;
    error_msg << "alloc_aligned failed with error message " << "\", errno \"" << errno << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
  ...
  {
    std::ostringstream error_msg;
    error_msg << "mmap failed with error message " << "\"" << strerror(errno) << "\", errno \"" << errno << "\"";
    QUILL_THROW(QuillError{error_msg.str()});
  }
