...
private:
  ...
  template <class U>
  FMTQUILL_CONSTEXPR static auto maybe_set_debug_format(U& u, bool set) -> decltype(u.set_debug_format(set))
  {
...
struct formatter<std::error_code, Char>
{
  ...
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto format(const std::error_code& ec,
...
{
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto format(const BitRef& v, FormatContext& ctx) const -> decltype(ctx.out())
  {
