...
class is_tuple_formattable_<T, C, true>
{
  ...
  static auto check(index_sequence<Is...>)
    -> decltype(check2(index_sequence<Is...>{},
...
struct formatter<Tuple, Char, enable_if_t<fmtquill::is_tuple_like<Tuple>::value && fmtquill::is_tuple_formattable<Tuple, Char>::value>>
{
private:
...
public:
  ...
  FMTQUILL_CONSTEXPR void set_separator(basic_string_view<Char> sep) { separator_ = sep; }

  FMTQUILL_CONSTEXPR void set_brackets(basic_string_view<Char> open, basic_string_view<Char> close)
  {
...
struct is_range
{
  static constexpr const bool value = detail::is_range_<T>::value &&
    !detail::is_std_string_like<T>::value && !std::is_convertible<T, std::basic_string<Char>>::value &&
...
struct range_mapper
{
  ...
  template <typename T, FMTQUILL_ENABLE_IF(has_formatter<remove_cvref_t<T>, Context>::value)>
 ...
 */
template <typename... T>
FMTQUILL_CONSTEXPR auto join(const std::tuple<T...>& tuple, string_view sep) -> tuple_join_view<char, T...>
{
  return {tuple, sep};
}
