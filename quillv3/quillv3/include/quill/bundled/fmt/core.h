...
#if (FMTQUILL_CPLUSPLUS >= 202002L || (FMTQUILL_CPLUSPLUS >= 201709L && FMTQUILL_GCC_VERSION >= 1002)) && \
...
// Check if constexpr std::char_traits<>::{compare,length} are supported.
  ...
  #endif
#elif defined(_LIBCPP_VERSION) && FMTQUILL_CPLUSPLUS >= 201703L && _LIBCPP_VERSION >= 4000
  #define FMTQUILL_CONSTEXPR_CHAR_TRAITS constexpr
...
// Check if exceptions are disabled.
  ...
  #if (defined(__GNUC__) && !defined(__EXCEPTIONS)) || (FMTQUILL_MSC_VERSION && !_HAS_EXCEPTIONS)
...
#if FMTQUILL_EXCEPTIONS && FMTQUILL_HAS_CPP_ATTRIBUTE(noreturn) && !FMTQUILL_MSC_VERSION && !defined(__NVCC__)
...
#ifdef _MSC_VER
  #define FMTQUILL_UNCHECKED_ITERATOR(It) using _Unchecked_type = It // Mark iterator as checked.
#else
...
// libc++ supports string_view in pre-c++17.
#if FMTQUILL_HAS_INCLUDE(<string_view>) && (FMTQUILL_CPLUSPLUS >= 201703L || defined(_LIBCPP_VERSION))
  #include <string_view>
  ...
  #define FMTQUILL_UNICODE !FMTQUILL_MSC_VERSION
    ...
    (defined(__cpp_consteval) && (!FMTQUILL_MSC_VERSION || FMTQUILL_MSC_VERSION >= 1929))
    ...
    ((FMTQUILL_GCC_VERSION >= 903 && FMTQUILL_CPLUSPLUS >= 201709L) || __cpp_nontype_template_args >= 201911L) && \
...
constexpr FMTQUILL_INLINE auto is_constant_evaluated(bool default_value = false) noexcept -> bool
{
// Workaround for incompatibility between libstdc++ consteval-based
// std::is_constant_evaluated() implementation and clang-14.
// https://github.com/fmtlib/fmt/issues/3247
#if FMTQUILL_CPLUSPLUS >= 202002L && defined(_GLIBCXX_RELEASE) && _GLIBCXX_RELEASE >= 12 &&        \
...
template <typename T>
constexpr FMTQUILL_INLINE auto const_check(T value) -> T
{
  return value;
}
...
// Do nothing.
#elif defined(__SIZEOF_INT128__) && !defined(__NVCC__) && !(FMTQUILL_CLANG_VERSION && FMTQUILL_MSC_VERSION)
...
FMTQUILL_CONSTEXPR auto to_unsigned(Int value) -> typename std::make_unsigned<Int>::type
{
...
FMTQUILL_CONSTEXPR inline auto is_utf8() -> bool
{
  ...
  return FMTQUILL_UNICODE || (sizeof(section) == 3 && uchar(section[0]) == 0xC2 && uchar(section[1]) == 0xA7);
}
} // namespace detail
...
public:
  ...
  FMTQUILL_CONSTEXPR basic_string_view(const std::basic_string<Char, Traits, Alloc>& s) noexcept
    : data_(s.data()), size_(s.size())
  {
  }

  template <typename S, FMTQUILL_ENABLE_IF(std::is_same<S, detail::std_string_view<Char>>::value)>
  FMTQUILL_CONSTEXPR basic_string_view(S s) noexcept : data_(s.data()), size_(s.size())
  {
  }
  ...
  FMTQUILL_CONSTEXPR_CHAR_TRAITS auto starts_with(basic_string_view<Char> sv) const noexcept -> bool
  {
  ...
  FMTQUILL_CONSTEXPR_CHAR_TRAITS friend auto operator==(basic_string_view lhs, basic_string_view rhs) -> bool
  {
...
template <typename Char, FMTQUILL_ENABLE_IF(!std::is_empty<std_string_view<Char>>::value)>
...
struct error_handler
{
  ...
  FMTQUILL_NORETURN void on_error(const char* message) { throw_format_error(message); }
};
} // namespace detail
...
public:
  explicit FMTQUILL_CONSTEXPR compile_parse_context(basic_string_view<Char> format_str,
                                                    int num_args,
                                                    const type* types,
                                                    int next_arg_id = 0)
...
FMTQUILL_CONSTEXPR auto copy_str(InputIt begin, InputIt end, OutputIt out) -> OutputIt
{
  while (begin != end)
...
template <typename Char, typename T, typename U, FMTQUILL_ENABLE_IF(std::is_same<remove_const_t<T>, U>::value&& is_char<U>::value)>
...
FMTQUILL_CONSTEXPR void basic_format_parse_context<Char>::do_check_arg_id(int id)
{
  ...
  if (detail::is_constant_evaluated() && (!FMTQUILL_GCC_VERSION || FMTQUILL_GCC_VERSION >= 1200))
  {
...
template <typename Char>
FMTQUILL_CONSTEXPR void basic_format_parse_context<Char>::check_dynamic_spec(int arg_id)
{
  if (detail::is_constant_evaluated() && (!FMTQUILL_GCC_VERSION || FMTQUILL_GCC_VERSION >= 1200))
  {
    using context = detail::compile_parse_context<Char>;
...
FMTQUILL_EXPORT template <typename Context>
...
template <typename T, typename Buf, FMTQUILL_ENABLE_IF(std::is_base_of<buffer<char>, Buf>::value)>
...
struct arg_data<T, Char, NUM_ARGS, 0>
{
  ...
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto named_args() -> std::nullptr_t { return nullptr; }
};
...
template <typename Char, typename T, typename... Tail, FMTQUILL_ENABLE_IF(!is_named_arg<T>::value)>
...
FMTQUILL_CONSTEXPR FMTQUILL_INLINE void init_named_args(std::nullptr_t, int, int, const Args&...)
{
}
...
public:
  ...
  FMTQUILL_INLINE value(const named_arg_info<char_type>* args, size_t size) : named_args{args, size}
  {
  }

  template <typename T>
  FMTQUILL_CONSTEXPR20 FMTQUILL_INLINE value(T& val)
  {
...
{
  template <typename U, FMTQUILL_ENABLE_IF(std::is_enum<U>::value || std::is_class<U>::value)>
...
struct arg_mapper
{
  ...
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(unsigned char val) -> unsigned { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(short val) -> int { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(unsigned short val) -> unsigned { return val; }
  ...
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(unsigned long val) -> ulong_type { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(long long val) -> long long { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(unsigned long long val) -> unsigned long long
  {
    return val;
  }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(int128_opt val) -> int128_opt { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(uint128_opt val) -> uint128_opt { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(bool val) -> bool { return val; }

  template <typename T, FMTQUILL_ENABLE_IF(std::is_same<T, char>::value || std::is_same<T, char_type>::value)>
  ...
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(long double val) -> long double { return val; }

  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(char_type* val) -> const char_type* { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(const char_type* val) -> const char_type*
  {
    return val;
  }
  template <typename T, FMTQUILL_ENABLE_IF(is_string<T>::value && !std::is_pointer<T>::value && std::is_same<char_type, char_t<T>>::value)>
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(const T& val) -> basic_string_view<char_type>
  {
  ...
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(const void* val) -> const void* { return val; }
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(std::nullptr_t val) -> const void* { return val; }

  // Use SFINAE instead of a const T* parameter to avoid a conflict with the
  // array overload.
  template <typename T,
            FMTQUILL_ENABLE_IF(std::is_pointer<T>::value || std::is_member_pointer<T>::value ||
  ...
  template <typename T, std::size_t N, FMTQUILL_ENABLE_IF(!std::is_same<T, wchar_t>::value)>
  ...
  template <typename T, typename U = format_as_t<T>, FMTQUILL_ENABLE_IF(std::is_arithmetic<U>::value)>
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(const T& val) -> decltype(FMTQUILL_DECLTYPE_THIS map(U()))
  {
  ...
  template <typename T,
            ...
            FMTQUILL_ENABLE_IF((std::is_class<U>::value || std::is_enum<U>::value || std::is_union<U>::value) &&
                               !is_string<U>::value && !is_char<U>::value &&
                               !is_named_arg<U>::value && !std::is_arithmetic<format_as_t<U>>::value)>
  FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto map(T& val) -> decltype(FMTQUILL_DECLTYPE_THIS do_map(val))
  {
    return do_map(val);
  }
...
private:
  ...
  friend FMTQUILL_CONSTEXPR auto detail::make_arg(T& value) -> basic_format_arg<ContextType>;
...
FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto visit_format_arg(Visitor&& vis,
...
public:
  ...
  FMTQUILL_CONSTEXPR auto arg_id(basic_string_view<Char> name) -> int { return args_.get_id(name); }
...
private:
  ...
  constexpr FMTQUILL_INLINE basic_format_args(unsigned long long desc, const detail::value<Context>* values)
...
public:
   ...
   */
  template <typename... Args>
  constexpr FMTQUILL_INLINE basic_format_args(const format_arg_store<Context, Args...>& store)
  ...
  /**
   ...
   \endrst
   */
  constexpr FMTQUILL_INLINE basic_format_args(const dynamic_format_arg_store<Context>& store)
...
public:
  ...
  FMTQUILL_CONSTEXPR auto operator[](size_t index) const -> const Char& { return data_[index]; }
};
} // namespace detail
...
struct arg_ref
{
  ...
  FMTQUILL_CONSTEXPR explicit arg_ref(int index) : kind(arg_id_kind::index), val(index) {}
...
FMTQUILL_CONSTEXPR auto do_parse_arg_id(const Char* begin, const Char* end, Handler&& handler) -> const Char*
{
  Char c = *begin;
  if (c >= '0' && c <= '9')
  {
...
template <typename Char, typename Handler>
FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto parse_arg_id(const Char* begin, const Char* end, Handler&& handler) -> const Char*
{
...
template <typename Char>
FMTQUILL_CONSTEXPR auto parse_dynamic_spec(const Char* begin,
                                           const Char* end,
                                           int& value,
                                           arg_ref<Char>& ref,
...
template <typename Char>
FMTQUILL_CONSTEXPR auto parse_precision(const Char* begin,
                                        const Char* end,
                                        int& value,
                                        arg_ref<Char>& ref,
                                        basic_format_parse_context<Char>& ctx) -> const Char*
...
template <typename Char>
FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto parse_format_specs(const Char* begin,
                                                           const Char* end,
...
template <typename Char, typename Handler>
FMTQUILL_CONSTEXPR auto parse_replacement_field(const Char* begin, const Char* end, Handler&& handler) -> const Char*
{
  struct id_adapter
  {
    Handler& handler;
    ...
    FMTQUILL_CONSTEXPR void on_name(basic_string_view<Char> id) { arg_id = handler.on_arg_id(id); }
  };
...
FMTQUILL_CONSTEXPR FMTQUILL_INLINE void parse_format_string(basic_string_view<Char> format_str, Handler&& handler)
{
...
FMTQUILL_CONSTEXPR auto parse_format_specs(ParseContext& ctx) -> decltype(ctx.begin())
{
...
public:
  ...
  FMTQUILL_CONSTEXPR auto on_arg_id(int id) -> int { return context_.check_arg_id(id), id; }
  ...
  FMTQUILL_CONSTEXPR auto on_format_specs(int id, const Char* begin, const Char*) -> const Char*
  {
  ...
  FMTQUILL_CONSTEXPR void on_error(const char* message) { throw_format_error(message); }
};
...
template <typename... Args, typename S, FMTQUILL_ENABLE_IF(is_compile_string<S>::value)>
...
public:
  ...
  template <detail::type U = detail::type_constant<T, Char>::value,
            FMTQUILL_ENABLE_IF(U == detail::type::string_type || U == detail::type::cstring_type || U == detail::type::char_type)>
...
*/
template <typename... T>
FMTQUILL_NODISCARD FMTQUILL_INLINE auto format(format_string<T...> fmt, T&&... args) -> std::string
{
  return vformat(fmt, fmtquill::make_format_args(args...));
}

/** Formats a string and writes the output to ``out``. */
template <typename OutputIt, FMTQUILL_ENABLE_IF(detail::is_output_iterator<OutputIt, char>::value)>
 ...
 **Example**::
 ...
 \endrst
 */
template <typename OutputIt, typename... T, FMTQUILL_ENABLE_IF(detail::is_output_iterator<OutputIt, char>::value)>
FMTQUILL_INLINE auto format_to(OutputIt out, format_string<T...> fmt, T&&... args) -> OutputIt
{
 ...
 */
template <typename OutputIt, typename... T, FMTQUILL_ENABLE_IF(detail::is_output_iterator<OutputIt, char>::value)>
FMTQUILL_INLINE auto format_to_n(OutputIt out, size_t n, format_string<T...> fmt, T&&... args) -> format_to_n_result<OutputIt>
{
...
template <typename... T>
FMTQUILL_NODISCARD FMTQUILL_INLINE auto formatted_size(format_string<T...> fmt, T&&... args) -> size_t
{
