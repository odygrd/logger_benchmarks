...
FMTQUILL_CONSTEXPR inline auto copy_str(InputIt begin, InputIt end, counting_iterator it) -> counting_iterator
{
  return it + (end - begin);
}
 ...
 */
  ...
  #define FMTQUILL_COMPILE(s) FMTQUILL_STRING_IMPL(s, fmtquill::detail::compiled_string, explicit)
#else
...
struct spec_field
{
  ...
  template <typename OutputIt, typename... Args>
  constexpr FMTQUILL_INLINE OutputIt format(OutputIt out, const Args&... args) const
  {
    const auto& vargs = fmtquill::make_format_args<basic_format_context<OutputIt, Char>>(args...);
...
template <typename... Args, typename S, FMTQUILL_ENABLE_IF(detail::is_compiled_string<S>::value)>
...
template <typename CompiledFormat, typename... Args, typename Char = typename CompiledFormat::char_type, FMTQUILL_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
FMTQUILL_INLINE std::basic_string<Char> format(const CompiledFormat& cf, const Args&... args)
{
...
template <typename OutputIt, typename CompiledFormat, typename... Args, FMTQUILL_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr FMTQUILL_INLINE OutputIt format_to(OutputIt out, const CompiledFormat& cf, const Args&... args)
{
  return cf.format(out, args...);
}

template <typename S, typename... Args, FMTQUILL_ENABLE_IF(detail::is_compiled_string<S>::value)>
FMTQUILL_INLINE std::basic_string<typename S::char_type> format(const S&, Args&&... args)
{
  ...
  {
    return fmtquill::format(static_cast<basic_string_view<typename S::char_type>>(S()),
...
template <typename OutputIt, typename S, typename... Args, FMTQUILL_ENABLE_IF(detail::is_compiled_string<S>::value)>
FMTQUILL_CONSTEXPR OutputIt format_to(OutputIt out, const S&, Args&&... args)
{
  constexpr auto compiled = detail::compile<Args...>(S());
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>, detail::unknown_format>())
  {
    return fmtquill::format_to(out, static_cast<basic_string_view<typename S::char_type>>(S()),
                               std::forward<Args>(args)...);
  }
  else
  {
...
auto format_to_n(OutputIt out, size_t n, const S& format_str, Args&&... args) -> format_to_n_result<OutputIt>
{
  ...
  fmtquill::format_to(std::back_inserter(buf), format_str, std::forward<Args>(args)...);
  return {buf.out(), buf.count()};
}

template <typename S, typename... Args, FMTQUILL_ENABLE_IF(detail::is_compiled_string<S>::value)>
FMTQUILL_CONSTEXPR20 auto formatted_size(const S& format_str, const Args&... args) -> size_t
{
  return fmtquill::format_to(detail::counting_iterator(), format_str, args...).count();
}
