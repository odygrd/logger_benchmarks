...
#include "format.h"
  ...
  #endif
  #if (FMTQUILL_HAS_INCLUDE(<fcntl.h>) || defined(__APPLE__) || defined(__linux__)) &&             \
...
namespace detail
{
FMTQUILL_API void format_windows_error(buffer<char>& out, int error_code, const char* message) noexcept;
}

FMTQUILL_API std::system_error vwindows_error(int error_code, string_view format_str, format_args args);
...
public:
  ...
  {
    vformat_to(std::back_inserter(buffer_), fmt, fmtquill::make_format_args(args...));
  }
};
