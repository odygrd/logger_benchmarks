...
struct rgb
{
  ...
  FMTQUILL_CONSTEXPR rgb(uint32_t hex) : r((hex >> 16) & 0xFF), g((hex >> 8) & 0xFF), b(hex & 0xFF)
  {
  }
...
struct color_type
{
  ...
  FMTQUILL_CONSTEXPR color_type(terminal_color term_color) noexcept : is_rgb(), value{}
  {
...
public:
  ...
  friend FMTQUILL_CONSTEXPR auto operator|(text_style lhs, const text_style& rhs) -> text_style
  {
    return lhs |= rhs;
  }

  FMTQUILL_CONSTEXPR auto has_foreground() const noexcept -> bool { return set_foreground_color; }
  FMTQUILL_CONSTEXPR auto has_background() const noexcept -> bool { return set_background_color; }
...
private:
  FMTQUILL_CONSTEXPR text_style(bool is_foreground, detail::color_type text_color) noexcept
  ...
  friend FMTQUILL_CONSTEXPR auto fg(detail::color_type foreground) noexcept -> text_style;

  friend FMTQUILL_CONSTEXPR auto bg(detail::color_type background) noexcept -> text_style;
...
FMTQUILL_CONSTEXPR inline auto fg(detail::color_type foreground) noexcept -> text_style
{
  return text_style(true, foreground);
}

/** Creates a text style from the background color. */
FMTQUILL_CONSTEXPR inline auto bg(detail::color_type background) noexcept -> text_style
{
  return text_style(false, background);
}

FMTQUILL_CONSTEXPR inline auto operator|(emphasis lhs, emphasis rhs) noexcept -> text_style
{
...
{
  FMTQUILL_CONSTEXPR ansi_color_escape(detail::color_type text_color, const char* esc) noexcept
  {
...
private:
  ...
  static FMTQUILL_CONSTEXPR void to_esc(uint8_t c, Char* out, char delimiter) noexcept
  {
  ...
  static FMTQUILL_CONSTEXPR auto has_emphasis(emphasis em, emphasis mask) noexcept -> bool
  {
...
template <typename Char>
FMTQUILL_CONSTEXPR auto make_foreground_color(detail::color_type foreground) noexcept -> ansi_color_escape<Char>
{
  return ansi_color_escape<Char>(foreground, "\x1b[38;2;");
}

template <typename Char>
FMTQUILL_CONSTEXPR auto make_background_color(detail::color_type background) noexcept -> ansi_color_escape<Char>
{
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename S, typename... Args, FMTQUILL_ENABLE_IF(detail::is_string<S>::value)>
void print(std::FILE* f, const text_style& ts, const S& format_str, const Args&... args)
{
  vprint(f, ts, format_str, fmtquill::make_format_args<buffer_context<char_t<S>>>(args...));
}
 ...
 */
template <typename OutputIt, typename Char, FMTQUILL_ENABLE_IF(detail::is_output_iterator<OutputIt, Char>::value)>
 ...
 */
template <typename T>
FMTQUILL_CONSTEXPR auto styled(const T& value, text_style ts) -> detail::styled_arg<remove_cvref_t<T>>
{
