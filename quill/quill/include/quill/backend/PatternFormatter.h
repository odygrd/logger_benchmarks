...
public:
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT std::string_view format(uint64_t timestamp,
                                                              ...
                                                              std::vector<std::pair<std::string, std::string>> const* structured_kvs,
...
private:
  ...
  QUILL_NODISCARD std::pair<std::string, std::array<size_t, PatternFormatter::Attribute::ATTR_NR_ITEMS>> _generate_fmt_format_string(
    std::bitset<PatternFormatter::Attribute::ATTR_NR_ITEMS>& is_set_in_pattern,
    std::string pattern,
    Args const&... args)
  {
